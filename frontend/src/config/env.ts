...
export async function fetchDeviceType(options?: {
  ...
  try {
    ...
    try {
      const authRes = await fetch(apiUrl("/api/auth/status"));
      if (authRes.ok) {
    ...
    const spendWait = (Boolean(token) || authBypassed) && !hardwareWaitSpent;
    ...
    while (res.ok && Date.now() < deadline) {
      ...
      if (peek.version === undefined && !authBypassed) {
        tokenRejected = true;
        break;
      }
    ...
    if (res.ok) {
      ...
      // SlothForge backend fallback: check /api/hardware
      try {
        const hwRes = await fetch(apiUrl("/api/hardware"));
        if (hwRes.ok) {
