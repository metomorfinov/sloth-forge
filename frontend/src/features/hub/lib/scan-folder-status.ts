...
function permissionHint(userAgent: string): string {
  switch (hostPlatform(userAgent)) {
    ...
    case "windows":
      return "Check the folder's security permissions, or allow SlothForge in Controlled Folder Access, then reopen this dialog.";
    default:
...
export function scanFolderStatusCopy(
  ...
  switch (status) {
    case "permission_denied":
      return {
        title: "SlothForge is not allowed to read this folder",
