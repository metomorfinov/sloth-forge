...
pub fn create_router(state: Arc<AppState>, static_dir: Option<PathBuf>) -> Router {
    ...
    let api_router = Router::new()
        ...
        // Training Endpoints
        // Обучение: жизненный цикл запусков, поток прогресса, история (training/)
        .route("/train/status", get(training::handlers::status))
        .route("/train/progress", get(training::handlers::progress).post(training::handlers::progress))
        .route("/train/start", post(training::handlers::start))
        .route("/train/stop", post(training::handlers::stop))
        .route("/train/reset", post(training::handlers::reset))
        .route("/train/runs", get(training::handlers::list_runs))
        .route("/train/runs/:id", get(training::handlers::get_run).patch(training::handlers::rename_run).delete(training::handlers::delete_run))
        .route("/train/metrics", get(training::handlers::metrics))
